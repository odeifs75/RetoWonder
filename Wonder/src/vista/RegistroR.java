...
public class RegistroR extends JDialog implements ActionListener {
	...
	public RegistroR(Dao dao, Usuario usu) {
		...
		comboQueBuscas.setModel(new DefaultComboBoxModel(new String[] {" Relacion estable", "Relacion pero ya veremos", "Rollos pero ya veremos", "Rollos cortos", "Hacer amigxs", "Aun no lo tengo claro"}));		comboQueBuscas.setSelectedIndex(-1);
		comboQueBuscas.setBounds(255, 97, 183, 22);
		contentPanel.add(comboQueBuscas);

		comboGustos = new JComboBox();
		comboGustos.setModel(new DefaultComboBoxModel(new String[] {"Videojuegos", "Fiesta", "Cine", "Motos", "Animales", "Escribir", "Cocina", "Futbol", "Gimnasio", "Reggaeton", "Viajar"}));
